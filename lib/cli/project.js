...
export default class Project {
  ...
  getBuildTree() {
    ...
    debug(`creating root builder at ${ buildPath }`);
    this.rootBuilder = new Builder(buildPath, this, preseededAddons);
    this.debugBuilderGraph([], 0, this.rootBuilder);
    let builders = [ ...this.builders.values() ];
    let trees = builders.map((builder) => builder.toTree());
    return new MergeTree(trees, { overwrite: true });
  }

  debugBuilderGraph(seenBuilders, level, builder) {
    debug(`${ padStart('', level, '  ') }   └─ ${ builder.pkg.name } -> ${ builder.dir }`);
    let seen = seenBuilders.includes(builder);
    seenBuilders.push(builder);
    if (!seen && builder.childBuilders) {
      builder.childBuilders.map(this.debugBuilderGraph.bind(this, seenBuilders, level + 1));
    }
  }
  ...
  builderFor(dir) {
    if (!this.builders.has(dir)) {
      ...
      debug(`using cached builder for ${ dir }`);
    }
  ...
  _finishBuild(results, outputDir) {
    ...
    eachDir(path.join(outputDir, 'node_modules'), (compiledAddon) => {
      let addonDependencies = path.join(this.dir, 'node_modules', compiledAddon, 'node_modules');
      if (isDir(addonDependencies)) {
        this._linkDependencies(addonDependencies, path.join(outputDir, 'node_modules', compiledAddon, 'node_modules'));
      }
    });
