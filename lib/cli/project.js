  ...
  build(outputDir = 'dist') {
    ...
    return broccoliBuilder.build()
      .then((results) => {
        ...
        spinner.text = `Build complete (${ elapsed }s)`;
        spinner.succeed();
        spinner.stop();
      }).then(() => {
        return outputDir;
  ...
  watchInDevelopmentChildAddons(childBuilders, onBuild) {
    ...
    if (inDevelopmentAddons.length > 0) {
      ...
      inDevelopmentAddons.forEach((childBuilder) => {
        let dir = fs.realpathSync(childBuilder.dir);
        debug(`"${ childBuilder.pkg.name }" (${ dir }) addon is under development, creating a project to watch & compile it`);
