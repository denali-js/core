...
export default class MemoryAdapter extends ORMAdapter {
  ...
  static async getRelated(model, relationship, descriptor) {
  ...
  static async setRelated(model, relationship, descriptor, relatedRecords) {
    if (descriptor.mode === 'hasMany') {
  ...
  static async addRelated(model, relationship, descriptor, relatedRecord) {
    model.record[`${ relationship }_ids`].push(relatedRecord.id);
  }

  static async removeRelated(model, relationship, descriptor, relatedRecord) {
