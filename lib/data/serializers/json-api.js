...
export default class JSONAPISerializer extends Serializer {
  ...
  static renderLinks(payload, document, options) {
    ...
    if (options.links) {
      ...
      let url = router.urlFor(this.action, payload);
      if (url) {
  ...
  static renderRecord(record, document, options = {}) {
    ...
    assert(serializedRecord.id, `Attempted to serialize a record (${ record }) without an id, but the JSON-API spec requires all resources to have an id.`);
  ...
  static includeRecord(config, descriptor, context) {
    ...
    let relatedSerializer = config.serializer || Serializer.serializerFor(record, relatedOptions);
    assert(relatedSerializer, `No serializer found for the ${ config.name } relationship. You must either supply a serializer as part of the relationship config object, or supply one for the ${ config.type } type in the serializers hash.`);
  ...
  static _parseResource(resource) {
    ...
    let relatedSerializer = Serializer.serializerFor(resource.type);
    relatedSerializer.parseResource(resource);
  }
