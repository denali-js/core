...
export default class Model {
  ...
  static async find(query, options) {
    debug(`${ this.type } find: ${ query }`);
    let records = await this.adapter.find(this.type, query, options);
   ...
   */
  set id(value) {
    return this.adapter.setId(this, id);
  }
  ...
  save(options) {
    return Promise.try(() => {
      debug(`saving ${ this.constructor.type }`);
      return this.adapter.saveRecord(this.constructor.type, this, options);
    }).return(this);
  }
   ...
   */
  delete(options) {
    return Promise.try(() => this.adapter.deleteRecord(this.constructor.type, this, options));
  }
  ...
  inspect() {
    ...
    return `<${ startCase(this.constructor.type) }:${ this.id == null ? '-new-' : this.id } ${ attributesSummary.join(', ') }>`;
  }
