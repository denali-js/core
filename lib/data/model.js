...
export default class Model {
  ...
  static create(data, options) {
    return Promise.try(() => {
      let record = this.adapter.buildRecord(this.type, {}, options);
      let instance = new this(record, options);
      ...
      get(model, property) {
        if (typeof property === 'string') {
          ...
          if (relatedMethodParts) {
            let [ _, operation, relationshipName ] = relatedMethodParts;
            ...
            if (descriptor && descriptor.isRelationship) {
              return model[`${ operation }Related`].bind(model, relationship, descriptor);
            }
          }
        }
  ...
  getRelated(relationshipName, options) {
    ...
    assert(descriptor && decsriptor.isRelationship, `You tried to fetch related ${ relationshipName }, but no such relationship exists on ${ this.constructor.type }`);
