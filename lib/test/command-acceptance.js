...
export default class CommandAcceptanceTest {
  ...
  constructor(command, options = {}) {
    ...
    this.projectRoot = path.dirname(path.dirname(findup('package.json')));
    ...
    if (!fs.existsSync(this.denaliPath)) {
      this.denaliPath = path.join(this.projectRoot, 'bin', 'denali');
    }
    ...
    if (options.populateWithDummy !== false) {
      ...
      // This is only necessary for denali itself, since it ends up a dependency
      // of itself (i.e. denali -> denali-babel -> denali). So we remove the
      // downloaded package so we can symlink in this version.
      rimraf.sync(path.join(this.dir, 'node_modules', projectName));
      // Symlink the addon under test as a dependency of this dummy app
      fs.symlinkSync(this.projectRoot, path.join(this.dir, 'node_modules', projectName));
    }
  }
  ...
  run(options = {}) {
    return new Promise((resolve, reject) => {
      exec(`${ this.denaliPath } ${ this.command }`, {
        ...
        if (options.failOnStderr && stderr.length > 0) {
          let error = new Error('Command printed to stderr, and failOnStderr was enabled');
