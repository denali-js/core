 ...
 * you've got easy to understand declarative validation. Or how about access
 * controls? Just add an `allow` array and a roles filter.
 *
 * @class Action
 * @constructor
 */
...
/**
 * The Action class has some special inheritance behavior. We use the `onExtend`
 * hook provided by Denali.Object to customize how subclasses are created. In
 * this case, we wrap any `before()` or `after()` methods in each mixin so that
 * each one is guaranteed to return a promise, and will automatically call
 * _super in the correct order so that filters will run from the first applied
 * mixin to the last applied.
 *
 * We use this hook rather than simply walking the Action's prototype chain at
 * invocation time because doing so would obscure the parent class `.before()`
 * of any mixin that was a subclass. Because getting the prototype of this
 * Action class would only return the subclass, and the next prototype would
 * _not_ be the subclass's parent class (it would be the previous mixin).
 * Visually:
 *
 *            Parent Class  << This class's `.before()` would be missed
 *            v
 *   Mixin -> Subclass Mixin -> Mixin = Action Class
 *
 * @method onExtend
 * @static
 * @for Action
 * @private
 */
Action.onExtend = function forceFilterSuper(mixin) {
  let modifications = {};
  if (mixin.before) {
    modifications.before = function enforceBeforeFilterSuper() {
      let previousFilter = this._super(...arguments);
      return Promise.resolve(previousFilter)
        .then(() => {
          let filter = mixin.before.call(this, ...arguments);
  ...
  if (mixin.after) {
    modifications.after = function enforceAfterFilterSuper() {
      let previousFilter = this._super(...arguments);
      return Promise.resolve(previousFilter)
        .then(() => {
          let filter = mixin.after.call(this, ...arguments);
          return Promise.resolve(filter);
        });
    };
  }
  return assign(modifications, mixin);
};
