 ...
 * @submodule runtime
 */
export default class Action extends DenaliObject {
  ...
  render(response) {
    ...
    debug(`[${ this.request.id }]: setting response status code to ${ response.status }`);
    this._response.statusCode = response.status;
    this._response.setHeader('X-Request-Id', this.request.id);

    if (response.body) {
      debug(`[${ this.request.id }]: writing response body`);
    ...
    this._response.end();
    debug(`[${ this.request.id }]: response finished`);
  }
  ...
  _buildFilterChains() {
    if (!this.constructor._before) {
      let prototypeChain = [];
      eachPrototype(this.constructor, (prototype) => {
        prototypeChain.push(prototype);
      });
      [ 'before', 'after' ].forEach((stage) => {
        let filterNames = compact(uniq(flatten(map(prototypeChain, stage))));
   ...
   * @private
   */
  async _invokeFilters(_chain, params) {
    let chain = _chain.slice(0);
    while (chain.length > 0) {
      let filterName = chain.shift();
      let filter = this[filterName];
      await Instrumentation.instrument('action.filter', {
        action: this.name,
        params,
        filter: filterName,
        headers: this.request.headers
