...
export default Addon.extend({
   ...
   * @returns {Router} an express Router that will handle incoming requests
   * @private
   */
  buildDispatcher() {
   ...
   *
   * @method buildRoutes
   * @returns {Router} an express Router with the routes attached
   * @private
   */
  buildRoutes() {
   ...
   * @returns {Router} an express Router with the middleware attached
   * @private
   */
  buildMiddleware() {
  ...
  buildErrorHandler() {
    return function(err, req, res, next) {
      const ErrorAction = this.container.lookup('action:error');
   ...
   * @param options.bind {Boolean} If false, the server will be created, but won't
   * actually bind to a port. Useful if you want a running Denali application that
   * isn't actually running on a port (i.e. testing, REPL usage, etc).
   ...
   */
  runInitializers() {
    const initializers = topsort(this.container.lookupAll('initializer'));
