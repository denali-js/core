...
export default Engine.extend({
  ...
  injectBlackburnPlusForaker(server) {
    ...
    server.use(blackburn({
      adapter: this.adapters.application,
    ...
    server.use((req, res, next) => {
      ...
      res.render = function(payload, options = {}) {
        let controller = req.context.controller;
        options.serializer = options.serializer || controller.serializer;
        return render.call(res, payload, options);
      };
      next();
    });
  },
