...
export default class Mailer {
  ...
  mail(options) {
    ...
    return sent.then(() => {
      this.logger.info(`mailer:${ name } - sent to ${ options.to }`);
    });
  }
  ...
  sendMail(options = {}) {
    ...
    return smtpTransport.sendMail(options, callback);
  }
  ...
  template(format, data) {
    if (!this.templateCache[format]) {
      ...
      let templateSrc = this.container.lookup('mailer:${ name }.${ extension }');
