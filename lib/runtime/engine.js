 ...
 */
  loadConfig(application) {
    ...
    forIn(process.env, (value, key) => {
      ...
      if (
        ...
        let normalizedKey = key.replace(/__/g, '.').split('_').slice(1).join('_');
  ...
  loadInitializers(application) {
    ...
    this.initializers = values(requireAll({
      dirname: initializersDir,
      resolve(initializerModule) {
  ...
  loadMiddleware(application) {
    let middleware = require(path.join(this.configDir, 'middleware'))
  ...
  loadRoutes(application) {
    let routes = require(path.join(this.configDir, 'routes'));
