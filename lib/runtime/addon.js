...
export default class Addon {
  ...
  /**
   * The express router that manages the routes for this addon
   *
   * @property
   * @type {express.Router}
   */
  ...
  get appDir () { return path.join(this.dir, 'app'); }
  ...
  discoverAddons() {
    ...
    forIn(this.pkg.dependencies, (version, pkgName) => {
      ...
      const pkgJSONPath = findup('package.json', { cwd: pkgMainPath });
      const pkgDir = path.dirname(pkgJSONPath);
      const pkg = require(pkgJSONPath);
      const isDenaliAddon = pkg.keywords && pkg.keywords.includes('denali-addon');
  ...
  loadApp() {
    eachDir(this.appDir, (dirname) => {
      ...
      glob.sync('**/*.js', { cwd: dir }).forEach((filepath) => {
        ...
        const mod = require(path.join(dir, filepath));
