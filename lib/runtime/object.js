...
DenaliObject.prototype.init = function init(options) {
  ...
  let typeMeta = metaFor(this.constructor);
  let meta = metaFor(this);
  typeMeta.instanceCount += 1;
  meta.typeMeta = typeMeta;
  meta.instanceId = typeMeta.instanceCount;
};

DenaliObject.toString = function() {
  let meta = metaFor(this);
  if (meta.typeMeta) {
    return `<${ meta.typeMeta.addon.name }:${ meta.typeMeta.modulepath }#${ meta.instanceId }`;
  }
  if (meta.addon) {
    return `[${ meta.addon.name }::${ meta.modulepath }:${ meta.id }]`;
  }
  return `[UnknownClass:${ meta.id }]`;
};

DenaliObject.prototype.toString = DenaliObject.toString;
