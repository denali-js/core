...
/**
 * onExtend lets a class customize how a mixin is applied to create a subclass.
 * It is called with each mixing that is applied, and should return the mixin
 * that should be applied instead.
 *
 * The onExtend method itself is statically inherited.
 *
 * @method function
 * @param mixin {Object} the mixin to be applied to the subclass
 * @return {Object} the mixin that should be applied instead
 * @static
 * @private
 */
DenaliObject.onExtend = function(mixin) {
  return mixin;
};
...
DenaliObject.extend = function(...mixins) {
  ...
  return mixins.reduce((ParentClass, mixin) => {
    ...
    // Allow for special extending logic
    ParentClass.onExtend(source);
