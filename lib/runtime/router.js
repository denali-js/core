 ...
 * @submodule runtime
 */
export default class Router extends DenaliObject {
  ...
  async handle(req, response) {
    ...
    try {
      ...
      await Promise.fromNode((cb) => this.middleware.run(req, response, cb));
  ...
  namespace(namespace, fn) {
    ...
    let wrapper = {
      get(pattern, ...args) {
        router.route('get', `${ namespace }/${ pattern.replace(/^\//, '') }`, ...args);
      },
      post(pattern, ...args) {
        router.route('post', `${ namespace }/${ pattern.replace(/^\//, '') }`, ...args);
      },
      put(pattern, ...args) {
        router.route('put', `${ namespace }/${ pattern.replace(/^\//, '') }`, ...args);
      },
      patch(pattern, ...args) {
        router.route('patch', `${ namespace }/${ pattern.replace(/^\//, '') }`, ...args);
      },
      delete(pattern, ...args) {
        router.route('delete', `${ namespace }/${ pattern.replace(/^\//, '') }`, ...args);
      },
      resource(resourceName, options) {
