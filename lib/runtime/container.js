...
export default class Container extends DenaliObject {
  ...
  _lookupOther(parsedName, options = {}) {
    ...
    if (!this._cache[parsedName.fullName]) {
      ...
      let alternate;
      if (!Class && options.fallback) {
        alternate = result(options.fallback);
        if (this._cache[alternate]) {
          return this._cache[alternate];
        }
        Class = this._registry[alternate];
      }
      // If the lookup and fallback failed, bail
      if (!Class) {
        let message = `No such ${ parsedName.type } found: '${ parsedName.moduleName }'`;
        if (alternate) {
          message += `. Fallback lookup '${ alternate }' was also not found.`;
        }
