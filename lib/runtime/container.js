...
export default DenaliObject.extend({
  ...
  lookupSerializer(name) {
    ...
    if (!serializer) {
      return this.lookupOther(this.parseName('serializer:application'));
    }
    this._ensureTypeInstanceMap('serializer');
    serializer.serializers = this._typeInstances.serializer;
    return serializer;
  },

  lookupAdapter(name) {
    let adapter = this.lookupOther(name);
    if (!adapter) {
      return this.lookupOther(this.parseName('adapter:application'));
    }
    this._ensureTypeInstanceMap('adapter');
    adapter.adapters = this._typeInstances.adapter;
    return adapter;
  },
  ...
  _lookupAll(type) {
    this._ensureTypeInstanceMap(type);
    return this._typeInstances[type];
  },

  _ensureTypeInstanceMap(type) {
